"""Config flow with menu-based options for Smart Garage v1.0."""
...
def _e(d): return selector.EntitySelector(selector.EntitySelectorConfig(domain=d if isinstance(d,list) else [d]))
def _n(mn,mx,s,u,m="box"): return selector.NumberSelector(selector.NumberSelectorConfig(min=mn,max=mx,step=s,unit_of_measurement=u,mode=selector.NumberSelectorMode.BOX if m=="box" else selector.NumberSelectorMode.SLIDER))
def _s(k): v=_SUG.get(k); return {"suggested_value":v} if v else {}

class SmartGarageConfigFlow(ConfigFlow, domain=DOMAIN):
    VERSION = 3
    def __init__(self): self._data = {}
    async def async_step_user(self, ui=None):
        if ui: self._data.update(ui); return await self.async_step_sensors()
    ...
    async def async_step_sensors(self, ui=None):
        if ui: self._data.update(ui); return await self.async_step_safety()
        return self.async_show_form(step_id="sensors", data_schema=vol.Schema({
    ...
    async def async_step_safety(self, ui=None):
        if ui: self._data.update(ui); return await self.async_step_ventilation()
        return self.async_show_form(step_id="safety", data_schema=vol.Schema({
    ...
    async def async_step_ventilation(self, ui=None):
        if ui:
            self._data.update(ui)
            if ui.get(CONF_ENABLE_VENTILATION): return await self.async_step_ventilation_sensors()
            return self.async_create_entry(title=self._data.get(CONF_NAME,"Smart Garage"), data=self._data)
        return self.async_show_form(step_id="ventilation", data_schema=vol.Schema({
            vol.Optional(CONF_ENABLE_VENTILATION, default=True): selector.BooleanSelector(),
        }))
    async def async_step_ventilation_sensors(self, ui=None):
        if ui: self._data.update(ui); return self.async_create_entry(title=self._data.get(CONF_NAME,"Smart Garage"), data=self._data)
        return self.async_show_form(step_id="ventilation_sensors", data_schema=vol.Schema({
    ...
    @staticmethod
    @callback
    def async_get_options_flow(entry): return SmartGarageOptionsFlow(entry)

class SmartGarageOptionsFlow(OptionsFlow):
    def __init__(self,entry): self._entry=entry; self._data=dict(entry.data)
    def _d(self,k,d=None): return self._data.get(k,d)
    def _save(self):
        self.hass.config_entries.async_update_entry(self._entry, data=self._data)
        return self.async_create_entry(title="", data={})
    async def async_step_init(self,ui=None):
        return self.async_show_menu(step_id="init", menu_options=["opt_basic","opt_sensors","opt_safety","opt_ventilation"])
    async def async_step_opt_basic(self,ui=None):
        if ui: self._data.update(ui); return self._save()
    ...
    async def async_step_opt_ventilation(self,ui=None):
        if ui:
            self._data.update(ui)
            if ui.get(CONF_ENABLE_VENTILATION): return await self.async_step_opt_vent_sensors()
            for k in [CONF_INDOOR_TEMP_SENSOR,CONF_INDOOR_HUMIDITY_SENSOR,CONF_OUTDOOR_TEMP_SENSOR,CONF_OUTDOOR_HUMIDITY_SENSOR,CONF_RAIN_SENSOR,CONF_HUMIDITY_THRESHOLD,CONF_AH_DIFF_THRESHOLD,CONF_VENTILATION_OPEN_S,CONF_VENTILATION_CHECK_INTERVAL,CONF_PRESENCE_ENTITY,CONF_RAIN_CLOSE_DELAY_MIN]:
